...
function printDiv(expense) {
	...
	<?php
		...
		if((isset($_GET['from']))&&!empty($_GET['to']))
		{
			...
			$sql_amount="select * from sec_expense where  committee_year='".$cur_academic_year."' and (exp_date BETWEEN '$from' and '$to') ORDER BY exp_date desc LIMIT $start_from, $num_rec_per_page ";
		   
		   
		   $sql_tot="select sum(amount) as total_amount from sec_expense where  committee_year='".$cur_academic_year."' and (exp_date BETWEEN '$from' and '$to') ORDER BY exp_date desc LIMIT $start_from, $num_rec_per_page";
			//var_dump($sql_tot);
		}
		...
		else
		{
			...
			$sql_amount="select * from sec_expense where  committee_year='".$cur_academic_year."' ORDER BY exp_date desc LIMIT $start_from, $num_rec_per_page ";
			
			$sql_tot="select sum(amount) as total_amount from sec_expense where committee_year='".$cur_academic_year."' ORDER BY exp_date desc LIMIT $start_from, $num_rec_per_page";
	...
	<?php
	$sql = "SELECT * FROM sec_expense where committee_year='".$cur_academic_year."'"; 
